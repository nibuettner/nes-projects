...
RESET:
  ...
  ;wait for VBLANK
:
  BIT $2002                              ;go back to prev unnamed label until $2002 is a 1
  BPL :-

  ;clear 2K memory
  TXA
CLEARMEMORY:                             ;clear from $0000 to $07FF
  STA $0000, X
  STA $0100, X
  STA $0300, X
  STA $0400, X
  ...
  STA $0200, X                           ;sprites
  LDA #$00
  INX                                    ;increase X, when it reaches #$FF and increases, it wil be #$00
  CPX #$00
  BNE CLEARMEMORY                        ;if not equal to #$00, continue loop
...
LOADSPRITES:
  ...
  STA $0200, X
  INX
  CPX #$20                               ;32 bytes (4 per sprite, 8 sprites total)
  BNE LOADSPRITES
...
;+-------- Flip sprite vertically
  ...
  ;mushroom
  ;Y Pos (px), SPRITE NUM, attributes, X Pos (px)
  ...
  ;sword
  ;Y Pos (px), SPRITE NUM, attributes, X Pos (px)
	.byte $40, $08, %00000001, $80
	.byte $40, $08, %01000001, $88
	.byte $48, $18, %00000001, $80
	.byte $48, $18, %01000001, $88
